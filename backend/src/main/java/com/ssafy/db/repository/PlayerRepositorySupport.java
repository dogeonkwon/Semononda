...
public class PlayerRepositorySupport {
	...
	public List<ReadyRes> gameStart(int gameConferenceRoomUid) {
		// return value
		ArrayList<ReadyRes> res = new ArrayList<>();
		// 방장의 UID
		int adminUid = jpaQueryFactory.select(qGameConferenceRoom.roomAdminUserUid).from(qGameConferenceRoom)
				.where(qGameConferenceRoom.uid.eq(gameConferenceRoomUid)).fetchOne();
		// 방장이 아닌 플레이어 리스트
		List<Player> player = jpaQueryFactory.selectFrom(qPlayer)
				.where(qPlayer.gameConferenceRoomUid.eq(gameConferenceRoomUid)).where(qPlayer.usersUid.ne(adminUid))
				.fetch();
		// 게임 시작 가능 여부
		...
		for (int i = 0; i < player.size(); i++) {
			// 플레이어가 준비 상태가 아닐 경우
			String userId = jpaQueryFactory.select(qUser.id).from(qUser).where(qUser.uid.eq(player.get(i).getUsersUid())).fetchFirst();
			if (!jpaQueryFactory.select(qPlayer.readyState).from(qPlayer).where(qPlayer.uid.eq(player.get(i).getUid()))
					.fetchFirst()) {
				allReady = false;
				res.add(ReadyRes.of(userId, false));
			} else {
				res.add(ReadyRes.of(userId, true));
			}
		}
		if (allReady) {
